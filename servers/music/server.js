...
exports.index = async (req, res, next) => {
  ...
  if (data.startsWith("http://") || data.startsWith("https://")) {
    // Thêm chức năng giải quyết URL rút gọn
    try {
      // Xác định nếu URL là kiểu rút gọn SoundCloud
      const resolvedUrl = await resolveShortUrl(data);
      const finalUrl = resolvedUrl || data;

      // Download functionality for direct URLs
      const stream = await soundcloudDownloader.download(finalUrl);
      ...
      if (downloadMode) {
        ...
        const info = await soundcloudDownloader.getInfo(finalUrl);
...
// Hàm mới để giải quyết các URL rút gọn
async function resolveShortUrl(url) {
  // Kiểm tra nếu URL là dạng on.soundcloud.com
  if (url.includes("on.soundcloud.com")) {
    return new Promise((resolve, reject) => {
      const protocol = url.startsWith("https:") ? https : http;

      const req = protocol.get(url, (res) => {
        // Nếu có chuyển hướng, trả về URL đích
        if (
          res.statusCode >= 300 &&
          res.statusCode < 400 &&
          ...
          // Nếu không có chuyển hướng, theo dõi body để tìm URL thực
          let data = "";
          res.on("data", (chunk) => {
            data += chunk;
          });
          ...
          res.on("end", () => {
            // Tìm URL canonical trong nội dung HTML
            ...
            if (canonicalMatch && canonicalMatch[1]) {
              ...
              // Không tìm thấy URL thật, trả về URL gốc
              resolve(url);
            }
          });
        }
      ...
      req.on("error", (err) => {
        ...
        resolve(url); // Trả về URL gốc nếu có lỗi
      });
  ...
  // Nếu không phải URL rút gọn, trả về null để sử dụng URL gốc
  return null;
}
